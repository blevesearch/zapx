...
func TestMaxTFNormCorrectness(t *testing.T) {
	...
	// Compute the expected value directly using wandTFNorm with the exact
	// fieldLen the norm column now records (no SmallFloat approximation).
	norm := normFromFieldLen(2)
...
func TestMaxTFNormSourceSidecar(t *testing.T) {
	...
	offset := pl.(*PostingsList).postingsOffset
	t.Logf("'some' FST offset=%d  encoding mask=%d", offset, offset&FSTValEncodingMask)

	// Inspect the raw sidecar records to see what postingsOffsets they contain.
	offsets, values := sb.maxTFNormColumns(uint16(fid))
	if len(offsets) == 0 {
		t.Fatal("maxTFNormColumns returned no offsets column")
	}
	if len(values)/maxTFNormValueSize != len(offsets)/maxTFNormOffsetSize {
		t.Fatalf("columns not parallel: %d offsets vs %d values",
			len(offsets)/maxTFNormOffsetSize, len(values)/maxTFNormValueSize)
	}
	t.Logf("sidecar for 'desc' has %d entries (addr=%d)", len(offsets)/maxTFNormOffsetSize, addr)

	mf, mfl, ok2 := sb.lookupMaxTFNorm(uint16(fid), offset)
	t.Logf("lookupMaxTFNorm(fid=%d, offset=%d): mf=%d  minFieldLen=%d  ok=%v", fid, offset, mf, mfl, ok2)
	if !ok2 {
...
func TestMaxTFNormMergeLookupOk(t *testing.T) {
	...
	// PostingsList resolves the merged segment's postingsOffset from the FST.
	pl, err := d.PostingsList([]byte("some"), nil, nil)
	if err != nil {
	...
	mf, mfl, ok := d.sb.lookupMaxTFNorm(d.fieldID, offset)
	if !ok {
	...
	if mfl == 0 {
		t.Error("lookupMaxTFNorm: minFieldLen == 0, expected > 0")
	}
...
func TestMaxTFNormMergeMaxTaken(t *testing.T) {
	...
	plo, errPL := d.PostingsList([]byte("alpha"), nil, nil)
	if errPL != nil {
		t.Fatalf("PostingsList: %v", errPL)
	}
	offset := plo.(*PostingsList).postingsOffset
	if offset&FSTValEncodingMask != FSTValEncodingGeneral {
	...
	// minFieldLen must come from the shortest doc (fieldLen=1 → highest norm),
	// exactly — the sidecar no longer quantizes it.
	if mfl != 1 {
		t.Errorf("minFieldLen = %d, want 1 (shortest doc across source segs)", mfl)
	}
}
...
func TestMaxTFNormMergeIdempotent(t *testing.T) {
	...
	plo, errPL := d.PostingsList([]byte("some"), nil, nil)
	if errPL != nil {
		t.Fatalf("PostingsList: %v", errPL)
	}
	offset := plo.(*PostingsList).postingsOffset
