...
func (vc *vectorIndexCache) loadOrCreate(fieldID uint16, mem []byte,
	...
	index *faiss.IndexImpl, vecDocIDMap map[int64]uint32, docVecIDMap map[int64][]uint32,
...
func (vc *vectorIndexCache) loadFromCache(fieldID uint16, loadDocVecIDMap bool,
	...
	docVecIDMap map[int64][]uint32, vecIDsToExclude []int64, err error) {
...
func (vc *vectorIndexCache) addDocVecIDMapToCacheLOCKED(ce *cacheEntry) map[int64][]uint32 {
	...
	docVecIDMap := make(map[int64][]uint32)
	for vecID, docID := range ce.vecDocIDMap {
		docVecIDMap[int64(docID)] = append(docVecIDMap[int64(docID)], uint32(vecID))
	}
...
func (vc *vectorIndexCache) createAndCacheLOCKED(fieldID uint16, mem []byte,
	...
	if loadDocVecIDMap {
		docVecIDMap = make(map[int64][]uint32, numVecs)
	}
	...
	for i := 0; i < int(numVecs); i++ {
		...
		if loadDocVecIDMap {
			docVecIDMap[int64(docID)] = append(docVecIDMap[int64(docID)],
				uint32(vecID))
		}
	}
...
func createCacheEntry(index *faiss.IndexImpl, vecDocIDMap map[int64]uint32,
	loadDocVecIDMap bool, docVecIDMap map[int64][]uint32, alpha float64) *cacheEntry {
	ce := &cacheEntry{
		index:       index,
...
func (ce *cacheEntry) load() (*faiss.IndexImpl, map[int64]uint32, map[int64][]uint32) {
	ce.incHit()
	ce.addRef()
