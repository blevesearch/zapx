...
func (sb *SegmentBase) InterpretVectorIndex(field string, requiresFiltering bool,
	...
	loadBinaryIndex := false
	if indexOptimisationType == uint64(index.SupportedVectorIndexOptimizations[index.IndexOptimizedForLatencyBinary]) ||
		indexOptimisationType == uint64(index.SupportedVectorIndexOptimizations[index.IndexOptimizedForRecallBinary]) {
		loadBinaryIndex = true
	}
