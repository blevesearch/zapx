		...
		for err == nil {
			...
			if !bytes.Equal(prevTerm, term) {
				...
				if sidecarsComplete && mergedPostingsOffset > 0 {
					mto.addEntry(fieldID, mergedPostingsOffset, mergedMaxFreq, mergedMinFieldLen)
				}
				sidecarsComplete = true
				mergedMaxFreq = 0
				mergedMinFieldLen = 0
			...
			if mf, mfl, ok := segmentsInFocus[itrI].lookupMaxTFNorm(srcFieldID, postingsOffset); ok {
				if mf > mergedMaxFreq {
					mergedMaxFreq = mf
				}
				// shortest field length across segments = highest norm
				if mfl > 0 && (mergedMinFieldLen == 0 || mfl < mergedMinFieldLen) {
					mergedMinFieldLen = mfl
				}
			} else {
