...
// sliceEligibleDocList is a minimal EligibleDocumentList backed by a uint64 slice.
type sliceEligibleDocList []uint64

func (s sliceEligibleDocList) Count() uint64 { return uint64(len(s)) }
func (s sliceEligibleDocList) Iterator() index.EligibleDocumentIterator {
	return &sliceEligibleDocIter{ids: s}
}

type sliceEligibleDocIter struct {
	ids sliceEligibleDocList
	pos int
}

func (it *sliceEligibleDocIter) Next() (uint64, bool) {
...
func TestSearchWithFilterAllVectorDocsEligible(t *testing.T) {
	...
	eligible := make(sliceEligibleDocList, numVecDocs)
	for i := range eligible {
		eligible[i] = uint64(numNonVecDocs + i)
	}

	vecSeg := segOnDisk.(segment.VectorSegment)
	vecIdx, err := vecSeg.InterpretVectorIndex(fieldName, nil)
	if err != nil {
		t.Fatal(err)
	}
	defer vecIdx.Close()
