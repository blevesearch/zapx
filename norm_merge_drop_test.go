...
//
// The norm column stores one exact uint32 field length per doc.  When a
// merge drops some docs, the merged segment must compact the norm column so that
// the surviving doc's field length is at the correct merged docNum — if a dropped
// doc's entry "shifts" the column incorrectly, every subsequent doc gets the wrong
...
// the four surviving docs reports its exact field length.
...
// (unique-term count) equals (startIdx+i+1) so each doc gets a distinct norm.
...
// norm column has the correct field length for each surviving doc.
//
// Source segments:
...
func TestNormColumnPreservedAfterMergeWithDrops(t *testing.T) {
	...
	for docNum, wantLen := range wantFieldLens {
		gotLen := pi.NormColumn(uint64(docNum))
		if gotLen != wantLen {
			t.Errorf("merged docNum %d: NormColumn=%d want %d (dropped doc shifted the column?)",
