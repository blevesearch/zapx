		...
		if fid < len(p.sb.fieldsSectionsMap) {
			...
			if SectionNormColumn < len(sm) {
				...
				if addr > 0 {
					...
					end := normStart + p.sb.numDocs*normColumnBytesPerDoc
...
type PostingsIterator struct {
	...
	// normColumn is a slice into sb.mem holding one LE uint32 field length per
	// doc for the field associated with this iterator (v18+). nil when
	// unavailable.
	normColumn []byte
...
func (i *PostingsIterator) readFreqNormHasLocs(docNum uint64) (uint64, uint64, bool, error) {
	...
	// v18: norm is in the column, not the freq stream.  The column holds the
	// exact analyzed field length, which is the same integer pre-v18 segments
	// stored (type-punned) as the freq stream's normBits.
	return freq, i.normColumnAt(docNum), hasLocs, nil
}

// normColumnAt returns the normBits (= exact field length) recorded for docNum,
// or 0 when this iterator has no norm column (pre-v18 segment, or a field
// without one).
func (i *PostingsIterator) normColumnAt(docNum uint64) uint64 {
	off := docNum * normColumnBytesPerDoc
	if off+normColumnBytesPerDoc <= uint64(len(i.normColumn)) {
		return uint64(binary.LittleEndian.Uint32(i.normColumn[off:]))
	}
	return 0
}
