...
// deterministic.
func sortedCellPairs(docs []geoTestDoc, cells func(geoTestDoc) []uint64) ([]uint64, []uint64) {
	var cellVals, docIDs []uint64
	for geoDocID, d := range docs {
		for _, cell := range cells(d) {
			cellVals = append(cellVals, cell)
			docIDs = append(docIDs, uint64(geoDocID))
		}
	}
...
func verifyGeoShapeV2Data(t *testing.T, geoData seg.GeoShapeV2Data,
	geoDocs []geoTestDoc, wantDocNums []uint64) {
	t.Helper()
...
func equalUint64Slices(got, want []uint64) bool {
	if len(got) != len(want) {
		return false
	}
	for i := range got {
...
func TestGeoIndexSectionRoundTrip(t *testing.T) {
	...
	verifyGeoShapeV2Data(t, geoData, geoDocs, []uint64{0, 2})
...
func TestGeoIndexMerge(t *testing.T) {
	...
	verifyGeoShapeV2Data(t, geoData, geoDocs, []uint64{0, 1, 3})
}
